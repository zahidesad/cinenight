    ...
    return (
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
            <div className="card p-6 flex flex-col justify-between">
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold text-white">{t('home.hero.title')}</h1>
                ...
                <div className="mt-6">
                    <label className="text-sm text-gray-400">{t('home.hero.search_label')}</label>
                    ...
                    {showDropdown && (
                        <div className="mt-3 card p-3 max-h-80 overflow-auto">
                            {isSearching && <SkeletonRow text={t('common.searching')} />}
                            ...
                            {!isSearching && !searchErr && results.length === 0 && <EmptyRow text={t('common.no_results')} />}

                            {!isSearching && !searchErr && results.length > 0 && (
                                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                                    {results.map((m: MovieDto) => (
                                        <MovieCard
                                            key={`s-${m.tmdbId}`}
                                            movie={m as never}
                                            variant="tmdb"
                    ...
                    <div className="flex-1 flex items-end bg-gradient-to-t from-black/80 to-transparent relative">
                        ...
                        <div className="relative p-5">
                            <h3 className="text-xl font-semibold text-white">
                                {featured?.title ?? featured?.name ?? t('home.hero.featured_movie')}
                            </h3>
                            {/* ... */}
                        </div>
                    </div>
